...
class DifficultySettings {
  ...
  static const settings = {
    Difficulty.gentle: DifficultySettings(threshold: 0.35, holdDuration: 1.5, fallSpeed: 100.0, spawnDelay: 4.0),
    Difficulty.standard: DifficultySettings(threshold: 0.55, holdDuration: 2.5, fallSpeed: 150.0, spawnDelay: 3.0),
    Difficulty.challenge: DifficultySettings(threshold: 0.75, holdDuration: 3.5, fallSpeed: 220.0, spawnDelay: 2.0),
  };
}
...
class _PianoGameScreenState extends State<PianoGameScreen> with SingleTickerProviderStateMixin {
  ...
  void _updateGame() {
    ...
    if (_lastSpawnTime > _settings.spawnDelay && _activeNotes.length < 3) {
      _lastSpawnTime = 0;
      _activeNotes.add(RibbonNote(
        lane: _random.nextInt(3),
        yPosition: -100, // Spawn above screen
        length: _settings.holdDuration * _settings.fallSpeed, // Length proportional to required hold
      ));
    }
    ...
    for (int i = 0; i < _activeNotes.length; i++) {
      ...
      if (note.state == RibbonState.falling || note.state == RibbonState.missed || note.state == RibbonState.broken) {
      ...
      bool isRelaxed = flexValue < (_settings.threshold * 0.6); // Release threshold
      ...
      // Hit detection
      if (note.state == RibbonState.falling && note.yPosition >= _hitZoneY - 30 && note.yPosition <= _hitZoneY + 30) {
        if (isHolding && _laneReadyToHit[note.lane]) {
          note.state = RibbonState.activeHold;
          note.lastHoldTime = now;
          _laneReadyToHit[note.lane] = false; // Must relax before hitting another
        }
      ...
      if (note.state == RibbonState.activeHold) {
        ...
        if (isHolding) {
          ...
          if (_gameLoop.value % 0.1 < 0.05) { // Occasional points tick
             _score += 10 + _combo;
          }
          ...
          if (note.lastHoldTime != null && now.difference(note.lastHoldTime!).inMilliseconds > 300) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Score: $_score', style: const TextStyle(color: Colors.white70)),
            Text('Max Combo: $_combo', style: const TextStyle(color: Colors.white70)),
            Text('Total Hold Time: ${_totalHoldDuration.toStringAsFixed(1)}s', style: const TextStyle(color: Colors.white70)),
            Text('Accuracy: ${_hits + _misses > 0 ? ((_hits / (_hits + _misses)) * 100).toStringAsFixed(1) : 0}%', style: const TextStyle(color: Colors.white70)),
          ],
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF0F172A), // Dark medical cyberpunk palette
      appBar: AppBar(
        title: const Text('Sustained Hold', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        ...
        actions: [
          ...
          IconButton(
            icon: const Icon(Icons.stop_circle_outlined, color: DesignTokens.errorColor),
          ...
          Positioned(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Combo: $_combo x', style: const TextStyle(color: DesignTokens.secondaryColor, fontSize: 20, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
              ...
              children: List.generate(3, (index) {
                return Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      color: _debugHolds[index] ? Colors.white.withOpacity(0.1) : Colors.transparent,
                      child: const Align(
                        alignment: Alignment.bottomCenter,
                        child: Padding(
                          padding: EdgeInsets.only(bottom: 20),
                          child: Text('HOLD (Debug)', style: TextStyle(color: Colors.white30)),
                        ),
                      ),
                    ),
                  ),
...
class PianoGamePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint hitZoneLinePaint = Paint()
      ...
      ..strokeWidth = 2;
    canvas.drawLine(Offset(0, hitZoneY), Offset(size.width, hitZoneY), hitZoneLinePaint);
    ...
    for (var note in notes) {
      ...
      if (note.state == RibbonState.missed || note.state == RibbonState.broken) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(tileRect, const Radius.circular(8)), tilePaint);
      canvas.drawRRect(RRect.fromRectAndRadius(tileRect, const Radius.circular(8)), borderPaint);
      ...
      if (note.state == RibbonState.activeHold) {
         ...
         canvas.drawArc(
           Rect.fromCircle(center: Offset(xCenter, note.yPosition - 30), radius: 25),
           -pi / 2, // Start at top
